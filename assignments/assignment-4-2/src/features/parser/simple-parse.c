...
static int is_first_of_arith_exp(const parse_data *data) {
  if (!data)
    return 0;
  return in_first_set(TOKEN_ID, data) || in_first_set(TOKEN_INTCON, data) ||
         in_first_set(TOKEN_LPAREN, data) || in_first_set(TOKEN_OPSUB, data);
}
...
static void report_syntax_error(parse_data *data, const char *message) {
  if (!data)
    return;
  if (!bad_exit(data)) {
    int line = data->current_token.line > 0 ? data->current_token.line : 0;
    const char *lexeme =
        data->current_token.lexeme ? data->current_token.lexeme : "";
...
parse_data *parse_func_defn(parse_data *data) {
  ...
  if (in_first_set(TOKEN_LBRACE, data)) {
    ...
    report_syntax_error(
        data, "Expected '{' to start function body or ';' for declaration");
    return data;
  }
  return data;
}
...
parse_data *parse_stmt_list(parse_data *data) {
  while (!bad_exit(data) &&
         (in_first_set(TOKEN_ID, data) || in_first_set(TOKEN_SEMI, data) ||
          in_first_set(TOKEN_KWIF, data) || in_first_set(TOKEN_KWWHILE, data) ||
...
parse_data *parse_stmt_after_id(parse_data *data) {
  if (in_first_set(TOKEN_OPASSG, data)) {
    ...
    if (does_not_match(TOKEN_SEMI, data->current_token)) {
      report_syntax_error(data,
                          "Expected semicolon ';' after assignment expression");
      return data;
    }
    update_data_to_next_token(data);
    ...
    if (does_not_match(TOKEN_SEMI, data->current_token)) {
      report_syntax_error(
          data, "Expected semicolon ';' after function call statement");
      return data;
    }
    update_data_to_next_token(data);
  } else {
    report_syntax_error(data, "Expected '=' for assignment or '(' for function "
                              "call after identifier");
  }
  return data;
}
...
parse_data *parse_return_stmt(parse_data *data) {
  ...
  if (in_first_set(TOKEN_SEMI, data)) {
    ...
    report_syntax_error(data,
                        "Expected semicolon ';' or expression after 'return'");
  }
  return data;
}
...
parse_data *parse_actuals_list_tail(parse_data *data) {
  while (!bad_exit(data) && in_first_set(TOKEN_COMMA, data)) {
    ...
    if (!is_first_of_arith_exp(data)) {
      report_syntax_error(data,
                          "Expected expression after comma in argument list");
      return data;
    }
    data = parse_arith_exp(data);
    if (bad_exit(data)) {
...
parse_data *parse_primary_exp(parse_data *data) {
  if (in_first_set(TOKEN_ID, data)) {
    ...
    report_syntax_error(
        data, "Expected identifier, integer constant, or '(' in expression");
  }
  return data;
}
...
parse_data *parse_term_prime(parse_data *data) {
  while (!bad_exit(data) &&
         (in_first_set(TOKEN_OPMUL, data) || in_first_set(TOKEN_OPDIV, data))) {
    update_data_to_next_token(data);
    data = parse_factor(data);
    if (bad_exit(data)) {
      return data;
...
parse_data *parse_arith_exp_prime(parse_data *data) {
  while (!bad_exit(data) &&
         (in_first_set(TOKEN_OPADD, data) || in_first_set(TOKEN_OPSUB, data))) {
    update_data_to_next_token(data);
...
parse_data *parse_relational_exp(parse_data *data) {
  ...
  int is_relop =
      in_first_set(TOKEN_OPLE, data) || in_first_set(TOKEN_OPEQ, data) ||
      in_first_set(TOKEN_OPNE, data) || in_first_set(TOKEN_OPLT, data) ||
      in_first_set(TOKEN_OPGE, data) || in_first_set(TOKEN_OPGT, data);
